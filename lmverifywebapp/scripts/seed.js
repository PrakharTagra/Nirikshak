 ...
 * Seeds standard Jurisdictions, LMPC Rules, and the primary CLM Controller account.
 * Safe to run multiple times without duplicating or overwriting existing users.
 *
...
async function main() {
  ...
  console.log(`   ✅ Rules ready: ${RULES.length} rules synced.`);

  console.log('\n3. Checking Controller (CLM) Account...');
  const existing = await User.findOne({ role: 'CLM' }).lean();
  if (existing) {
    console.log(`   ℹ️ Controller account already exists: ${existing.username}`);
    console.log('   (Skipping user creation. Delete this user document if you wish to re-seed.)');
    return;
  }
  ...
  console.log('\n   🎉 Controller root account created successfully!');
  console.log(`   Username:  ${CLM_USERNAME}`);
  console.log(`   Password:  ${password}`);
  console.log('\n   ⚠️  Please record this initial password. It will not be shown again.');
  console.log('   You will be asked to replace it upon initial sign-in.\n');
}
