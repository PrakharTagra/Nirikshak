...
export function PdfLink({ url, label = 'Open report PDF' }) {
  if (!url) return <span className="text-xs text-slate-400 italic">No PDF Attached</span>;
  return (
    <a
      href={url}
      target="_blank"
  ...
  return (
    <section className="border border-slate-300 bg-white border-t-[3px] border-t-govt-navy shadow-xs rounded-xs overflow-hidden">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-[#f8f9fa] px-4 py-3">
        <div>
          <h2 className="text-sm font-bold text-govt-dark tracking-wide uppercase">{title}</h2>
...
export function Loading({ label = 'Loading records' }) {
  return (
    <div className="px-4 py-16 flex flex-col items-center justify-center gap-3 text-sm text-slate-600" role="status">
      <div className="w-8 h-8 border-3 border-slate-200 border-t-govt-navy border-r-saffron rounded-full animate-spin" />
      ...
      <ol className="flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
        ...
        {items.map((item, index) => (
          <li key={index} className="flex items-center gap-1.5">
            ...
            {item.to ? (
              <Link to={item.to} className="hover:text-govt-navy font-medium transition-colors">
                {item.label}
              </Link>
            ) : (
