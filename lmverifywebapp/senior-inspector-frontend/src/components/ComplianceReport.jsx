...
export default function ComplianceReport({ report, officer }) {
  let authUser = null;
  try {
    const auth = useAuth?.();
    authUser = auth?.user;
  } catch (_) {}
  const user = officer || authUser || {};
  ...
  const declarations = report.declarations || report.declared_values || report.compliance?.declarations || {};
  const packageRecord = report.packageRecord || report.package_record || {};
  ...
  const inspectorName =
    ...
    user?.name ||
    user?.full_name ||
    "Digital Marketplace Officer";
  const inspectorRole =
    officer?.role ||
    report.officer_role ||
    "Digital Marketplace Inspector (DMI)";
  const inspectorJurisdiction =
    report.jurisdiction ||
    user?.jurisdiction ||
    "Central E-Commerce Surveillance Unit";
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      await generatePdfReport(report, { name: inspectorName, role: inspectorRole, ...user });
    } catch (err) {
